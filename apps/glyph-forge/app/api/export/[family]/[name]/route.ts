...
import { CIRCULAR_WEIGHTS, cellKey, type Family } from "@/lib/data";
import {
  loadCellScores,
  loadGlyphScores,
  loadManifest,
...
type Params = { family: string; name: string };
...
function scoreBadge(composite: number | null | undefined): {
  ...
  if (composite < 0.3) return { label: `${pct}`, color: "#e04e5e" };
  if (composite < 0.7) return { label: `${pct}`, color: "#f0a020" };
  return { label: `${pct}`, color: "#3ba963" };
}
...
export async function GET(
  ...
  const cellRows = await Promise.all(
    CIRCULAR_WEIGHTS.map(async (w) => {
      const donor = await svgInline(family, name, w.wght, "donor");
      const glide = await svgInline(family, name, w.wght, "glide");
      const s: CellScores | undefined =
        cellScores?.[cellKey(family, name, w.wght)];
      return { w, donor, glide, s };
    })
  );

  const title = `${family}/${name} — Glyph Forge export`;
      ...
      .map(({ w, donor, glide, s }) => {
        const inner =
          source === "overlay"
            ...
            : (source === "donor" ? donor : glide) ?? '<span class="empty">—</span>';
        const scoreTag =
          source === "overlay" && s
