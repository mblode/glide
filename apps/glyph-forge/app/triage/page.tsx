...
import type { BrokenGlyph } from "@glide/glyph-forge-engine";
...
export const metadata = { title: "Triage queue — Glyph Forge" };

export default async function TriagePage() {
  const [manifest, pending] = await Promise.all([loadManifest(), readPending()]);
  const byKey = new Map<string, BrokenGlyph>();
  for (const g of manifest) byKey.set(`${g.family}/${g.name}`, g);
  ...
  const rows = pending
    ...
    .sort((a, b) => keyOf(a.pending).localeCompare(keyOf(b.pending)));

  const counts: Record<string, number> = {};
  for (const p of pending) counts[p.strategy] = (counts[p.strategy] ?? 0) + 1;
    ...
    <main>
      <header className="border-b border-border px-6 py-4">
        <div className="mx-auto flex max-w-[1600px] items-center justify-between gap-4">
          <div>
            <h1 className="font-mono text-lg tracking-tight">Triage queue</h1>
            <p className="mt-0.5 text-xs text-muted-foreground">
              {pending.length} staged edit{pending.length === 1 ? "" : "s"} · not
              yet applied to{" "}
          ...
          <Button asChild variant="outline" size="sm">
            <Link href="/">← back to grid</Link>
          </Button>
        </div>
        {pending.length > 0 && (
          <div className="mx-auto mt-3 flex max-w-[1600px] flex-wrap items-center gap-2 font-mono text-xs text-muted-foreground">
            ...
            {Object.entries(counts)
              ...
              .map(([strat, n]) => (
                <span
                  key={strat}
                  className="rounded bg-card px-2 py-0.5"
                >
                  {strat} · {n}
                </span>
              ))}
        ...
        {pending.length > 0 && (
          <div className="mt-6 rounded-lg border border-border bg-background p-4 text-xs">
            ...
            <p className="mt-2 text-muted-foreground">
              The UI never writes to{" "}
              <code>circular-triage.json</code> directly. Review the diff and
              merge via CLI:
            </p>
...
npm --workspace @glide/glyph-forge-engine run apply -- --dry-run

# write to circular-triage.json (auto-backs up the current file)
npm --workspace @glide/glyph-forge-engine run apply`}
            </pre>
          </div>
        )}
      </div>
