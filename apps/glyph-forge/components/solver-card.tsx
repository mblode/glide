...
import { scoreBand, type Family } from "@/lib/data";
...
function Tile({
  ...
  const params = candidate.params
    ? Object.entries(candidate.params)
        .filter(([k]) => k !== "referenceWght" || candidate.strategy === "reference_fallback")
        .map(([k, v]) => `${k} ${v}`)
        .join(" · ")
    : "";
  return (
        ...
        <>
          ...
          {stagedLocal ? (
            <p className="rounded border border-[--color-band-amber]/30 bg-[--color-band-amber]/10 px-2 py-1.5 font-mono text-[11px] text-[--color-band-amber]">
              ✓ Staged — review on <a href="/triage" className="underline">/triage</a>
            </p>
          ) : (
            <button
              type="button"
            ...
            >
              {isPending ? "Staging…" : `↗ Stage solver verdict (${CAND_LABEL[verdict.best]})`}
            </button>
          )}
        </>
      ) : (
        <p className="text-xs text-muted-foreground/70">
          No candidate produced a scorable projection — likely missing from donor
          or glide at all weights.
        </p>
      )}
