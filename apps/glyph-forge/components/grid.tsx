...
import { FilterBar, type GridFilters, type SortKey } from "./filter-bar";
import { GlyphCard } from "./glyph-card";

export function Grid({ glyphs }: { glyphs: ScoredGlyph[] }) {
  const allFeatures = useMemo(() => {
    const s = new Set<string>();
    for (const g of glyphs) for (const f of g.features) s.add(f);
    return Array.from(s).sort();
  }, [glyphs]);

  const familyCounts = useMemo(() => {
    const c = { roman: 0, italic: 0 };
    for (const g of glyphs) c[g.family] += 1;
    return c;
  }, [glyphs]);
  ...
  const visible = useMemo(() => {
    ...
    const filtered = glyphs.filter((g) => {
      ...
      if (filters.bands.size > 0) {
        ...
        if (!filters.bands.has(band)) return false;
      }
