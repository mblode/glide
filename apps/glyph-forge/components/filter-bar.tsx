...
import type { ScoreBand } from "@glide/glyph-forge-engine";
    ...
    <div className="sticky top-0 z-10 border-b border-border bg-background/90 backdrop-blur">
      <div className="mx-auto flex max-w-[1600px] flex-wrap items-center gap-3 px-6 py-3 text-sm">
        <div className="flex items-center gap-1">
          ...
          >
            Italic <span className="text-muted-foreground/70">{familyCounts.italic}</span>
          </FilterChip>
          <FilterChip
            active={filters.family === "roman"}
            onClick={() => setFilters({ ...filters, family: "roman" })}
          >
            Roman <span className="text-muted-foreground/70">{familyCounts.roman}</span>
          </FilterChip>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-1">
          {VERDICT_ORDER.map((v) => (
            <FilterChip
              ...
              onClick={() =>
                setFilters({ ...filters, verdicts: toggle(filters.verdicts, v) })
              }
            >
              {VERDICT_LABEL[v]}
            </FilterChip>
        ...
        {hasScores && (
          <>
            <Divider />
            <div className="flex flex-wrap items-center gap-1">
              {(["red", "amber", "green", "unknown"] as ScoreBand[]).map((b) => (
                <FilterChip
                  key={b}
                  active={filters.bands.has(b)}
                  onClick={() =>
                    setFilters({ ...filters, bands: toggle(filters.bands, b) })
                  }
                >
                  <span
        ...
        >
          {hasScores && <option value="score-asc">Sort: worst score first</option>}
          {hasScores && <option value="score-desc">Sort: best score first</option>}
          {hasSolver && <option value="gain-desc">Sort: biggest solver gain</option>}
        ...
        <div className="mx-auto flex max-w-[1600px] flex-wrap items-center gap-1 px-6 pb-3 text-xs">
          ...
          {allFeatures.map((f) => (
            <FilterChip
              ...
              onClick={() =>
                setFilters({ ...filters, features: toggle(filters.features, f) })
              }
              compact
            >
              {f}
