...
def rectangle_ops(left: float, bottom: float, right: float, top: float, clockwise: bool) -> list[tuple[str, tuple]]:
    if clockwise:
        points = [
            (left, bottom),
            (left, top),
...
def apply_reference_fallback(font, glyph_name: str, reference_master_name: str) -> dict[str, object]:
    ...
    if (
        ...
        if mapping is not None:
            ...
            for index, (reference_contour, target_contour) in enumerate(
                zip(reference_contours, ordered_target)
            ):
...
def bracketing_masters_for_weight(font, target_weight: int) -> tuple[object, object, float] | None:
    ...
    if target_weight <= master_weight(masters[0]) or target_weight >= master_weight(masters[-1]):
        return None
    for left, right in zip(masters, masters[1:]):
...
def donor_ops_for_master(glyph, master, donor_data_by_master_id: dict[str, tuple]) -> list[tuple[str, tuple]] | None:
    ...
    if (
        ...
        if left_mapping is not None and right_mapping is not None:
            ...
            for index, (reference_contour, left_contour, right_contour) in enumerate(
                zip(reference_contours, ordered_left, ordered_right)
            ):
...
def interpolate_compatible_ops(
    ...
    for (left_op, left_args), (right_op, right_args) in zip(left_ops, right_ops):
        ...
        interpolated_args = tuple(
            ...
            for left_point, right_point in zip(left_args, right_args)
        )
...
def align_layer_paths_to_reference(reference_layer, layer) -> dict[str, object]:
    ...
    for path_index, (reference_path, other_path) in enumerate(zip(reference_paths, paths)):
    ...
    for weight in sorted({int(value) for value in brace_weights}):
        ...
        if brace_mode == "direct_donor_interpolation":
            ...
            if donor_pair is None or not verify_cubic_compat(donor_pair):
                ...
                continue
            donor_interpolated_ops = interpolate_compatible_ops(donor_pair["left"], donor_pair["right"], t)
            ...
            for index, (reference_contour, donor_contour) in enumerate(
                zip(reference_contours, ordered_donor)
            ):
...
def scale_ops(
    ...
    for op, args in ops:
        scaled_args = tuple(
            tuple(float(value) * upm_scale for value in point)
            for point in args
        )
...
def apply_structural_fallback(font, glyph_name: str, reference_master_name: str, plan) -> dict[str, object]:
...
def apply_localized_substitution(
    ...
    contour_index_set = {
        ...
        if 0 <= int(index) < len(reference_contours)
    }
...
def audit_direction_issues(font) -> dict[str, int]:
    ...
    for glyph in font.glyphs:
        ...
        for other_layer in ordered_layers[1:]:
            ...
            for ref_path, other_path in zip(ref_paths, ordered_other_paths):
                ref_sign = sign(path_signed_area(ref_path))
                other_sign = sign(path_signed_area(other_path))
                if ref_sign == 0 or other_sign == 0 or ref_sign == other_sign:
                    continue
...
def build_family_source_report(
    ...
    path_order_index: Counter[str] = Counter(glyph_name for glyph_name, _, _ in strict_audit["path_order_issues"])
    node_count_index: Counter[str] = Counter(glyph_name for glyph_name, _, _, _, _ in strict_audit["node_count_issues"])
    start_index: Counter[str] = Counter(glyph_name for glyph_name, _, _, _, _ in strict_audit["start_issues"])
    ...
    for glyph in font.glyphs:
        ...
        for master in font.masters:
            ...
            if donor_ops:
                ...
                donor_segment_map[master.name] = [count_segments(contour) for contour in parse_contours(donor_ops)]
            else:
                donor_segment_map[master.name] = []
        ...
        for master in font.masters:
            ...
            if len(donor_segments) != len(source_segments):
                ...
                continue
            deltas = [source - donor for donor, source in zip(donor_segments, source_segments)]
        ...
        entries.append(
            {
                ...
                "source_path_order_issues": 0 if has_source_path_override else path_order_index.get(glyph.name, 0),
                "source_node_count_issues": node_count_index.get(glyph.name, 0),
                "source_start_issues": 0 if has_source_path_override else start_index.get(glyph.name, 0),
                "source_direction_issues": 0 if has_source_path_override else direction_index.get(glyph.name, 0),
...
def _best_ufo_contour_mapping(reference_contours, other_contours) -> list[int]:
    ...
    reference_signatures = [_ufo_canonical_contour_signature(contour) for contour in reference_contours]
...
def sample_weights(family_manifest: dict[str, object], font) -> list[int]:
    ...
    weights = sorted(int(master.axes[0]) for master in font.masters if getattr(master, "axes", None))
    samples = set(weights)
...
def generate_static_samples(variable_font_path: Path, weights: list[int], output_dir: Path) -> dict[int, Path]:
...
def compare_fonts(instance_path: Path, donor_path: Path, glyph_subset: set[str] | None = None) -> dict[str, object]:
    ...
    return {
        ...
        "worst_deviations": dict(sorted(deviations.items(), key=lambda item: item[1], reverse=True)[:20]),
        "worst_area_diffs_pct": dict(sorted(area_diffs.items(), key=lambda item: item[1], reverse=True)[:20]),
    }


def sample_quadratic(p0: tuple[float, float], p1: tuple[float, float], p2: tuple[float, float], steps: int) -> list[tuple[float, float]]:
    points = []
...
def glyph_polylines(font: TTFont, glyph_name: str, steps: int = 10) -> list[list[tuple[float, float]]]:
...
def segments_for_contours(contours: list[list[tuple[float, float]]]) -> list[tuple[int, int, tuple[float, float], tuple[float, float]]]:
    segments = []
    for contour_index, contour in enumerate(contours):
        for segment_index in range(len(contour) - 1):
            segments.append((contour_index, segment_index, contour[segment_index], contour[segment_index + 1]))
    return segments
...
def segments_intersect(a0, a1, b0, b1) -> bool:
    ...
    d3 = ccw(b0, b1, a0)
    d4 = ccw(b0, b1, a1)
    return (d1 == 0 or d2 == 0 or sign(d1) != sign(d2)) and (d3 == 0 or d4 == 0 or sign(d3) != sign(d4))
    ...
    for index, (contour_a, segment_a, a0, a1) in enumerate(segments):
        for contour_b, segment_b, b0, b1 in segments[index + 1:]:
            if contour_a == contour_b and abs(segment_a - segment_b) <= 1:
                continue
            if contour_a == contour_b and {segment_a, segment_b} == {0, len(contours[contour_a]) - 2}:
                continue
...
def build_validation_report(
    ...
    for weight, donor_path in donor_weights.items():
        ...
        if instance_path is None:
            continue
        payload["weights"][str(weight)] = compare_fonts(instance_path, donor_path, glyph_subset=review_glyphs)
...
def repair_bucket_configs(glyph_configs: dict[str, dict[str, object]]) -> dict[str, dict[str, object]]:
    return {
        glyph_name: config
...
def instance_risk_details_for_glyph(
    ...
    for weight, payload in instance_payload.get("weights", {}).items():
        ...
        if value is not None:
            min_segment_length = value if min_segment_length is None else min(min_segment_length, value)
        ...
        entries.append(
            {
                ...
                "interpolatable": interpolatable_details_for_glyph(interpolatable_payload, glyph_name),
...
def recommend_exception_action(entry: dict[str, object]) -> dict[str, object]:
    ...
    if repair_bucket == "freeze_candidate":
        ...
        if max_intersections > 0 or max_area_diff >= 4.0 or total_segment_delta >= 5 or "kink" in issue_types:
            action = "manual_redraw_required"
            confidence = "high"
        else:
            action = "localized_substitution"
...
def build_exception_plan_report(
    ...
    summary = {
        "recommended_action_counts": dict(Counter(action["recommended_action"] for action in actions)),
...
def build_review_packet(
    ...
    for family_key in ("roman", "italic"):
        ...
        lines.append(f"- classification counts: `{json.dumps(summary['classification_counts'], sort_keys=True)}`")
        lines.append(f"- interpolatable problem glyphs: `{interpolatable_reports[family_key]['problem_glyphs']}`")
...
def run_family(
    ...
    interpolatable_payload = run_interpolatable_designspace(designspace_path, interpolatable_report_path)
    ...
    build_payload.update(
        {
            ...
            "cluster_evidence_report": str(cluster_evidence_report_path) if cluster_evidence_report_path else None,
            "cluster_plan_report": str(cluster_plan_report_path) if cluster_plan_report_path else None,
        }
    )
    return build_payload
...
def main() -> int:
    ...
    if not args.skip_build and set(selected) == {"roman", "italic"}:
        source_reports = {
            family_key: report_dir / f"{family_key}-source-report.json"
            for family_key in selected
        }
        instance_reports = {
    ...
    summary_path.write_text(json.dumps(family_results, indent=2))
