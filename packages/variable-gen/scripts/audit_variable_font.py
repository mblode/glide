...
from repair_circular_sources import (
    build_variable_font,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            "Number of interior sample weights to generate inside each adjacent "
            "master span."
        ),
    )
    parser.add_argument(
    ...
    for glyph_name in glyph_order:
        ...
        issue_types = sorted(
            Counter(
                ...
                for issue in interpolatable_issues
            ).items()
        )
...
def build_interpolation_focus_summary(
    ...
    return focus_summary, {
        **dict(summary_counts),
        "span_problem_glyphs": {
            span_label: int(span_problem_counts.get(span_label, 0))
...
def run_family_audit(
    ...
    return {
        ...
        "interpolatable_summary": interpolatable_summary,
        "summary": (
            interpolation_focus_summary if interpolation_only else summary
        ),
        "comprehensive_summary": summary,
        "interpolation_focus_summary": interpolation_focus_summary,
    }
...
def build_overview_report(
    ...
    for family_key, result in results.items():
        ...
        lines.append(f"- interpolatable: `{json.dumps(result['interpolatable_summary'], sort_keys=True)}`")
        lines.append(
...
def main() -> int:
    ...
    mode_slug = audit_mode_slug(args.interpolation_only)
    summary_name = (
    ...
    summary_path.write_text(json.dumps(json_safe(results), indent=2))
    overview_name = (
        "audit-overview.md"
        if not args.interpolation_only
        else f"audit-overview-{mode_slug}.md"
    )
    overview_path = build_overview_report(results, report_dir, output_name=overview_name)
