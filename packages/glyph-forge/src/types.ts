...
export type SvgCellPath = `${Family}/${string}/${number}-${GlyphCellSource}.svg`;
...
export function scoreBand(composite: number | null | undefined): ScoreBand {
  if (composite === null || composite === undefined) return "unknown";
  if (composite < 0.3) return "red";
  if (composite < 0.7) return "amber";
  return "green";
}
