...
from shared import CACHE_DIR, CIRCULAR_WEIGHTS, MANIFEST_PATH, Family
...
def build(force: bool, limit: int | None, only_seed: bool, only_verdicts: set[str] | None) -> int:
    ...
    for i, entry in enumerate(glyphs):
        family = entry["family"]
        name = entry["name"]
        for weight in CIRCULAR_WEIGHTS:
    ...
    elapsed = time.monotonic() - start
    print(
        f"done: {written} written, {skipped} skipped, {missing} missing in {elapsed:.1f}s"
    )
...
def main() -> int:
    ...
    parser.add_argument(
        "--only-seed",
        action="store_true",
        help="Only render glyphs from ITALIC_SEED / ROMAN_SEED (skip audit-only glyphs).",
    )
    parser.add_argument(
        "--only-verdicts",
        nargs="+",
