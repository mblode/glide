...
from shared import CIRCULAR_WEIGHTS, CellSource, Family, circular_otf, glide_vf, resolve_glyph_name
...
def _flatten_ops(ops: list, steps: int = 12) -> list[tuple[float, float]]:
    ...
    for op, args in ops:
        if op == "moveTo":
            ...
            on_pts = [current, *args]
            for a, b, c in zip(on_pts, on_pts[1:], on_pts[2:]):
                # midpoint trick: quads are between implied midpoints
                start = ((a[0] + b[0]) / 2.0, (a[1] + b[1]) / 2.0) if on_pts.index(a) > 0 else a
                control = b
                end = ((b[0] + c[0]) / 2.0, (b[1] + c[1]) / 2.0) if on_pts.index(c) < len(on_pts) - 1 else c
                for i in range(1, steps + 1):
                    t = i / steps
                    u = 1 - t
...
def _raster(ops: list, font: TTFont) -> np.ndarray:
    ...
    for sub in _subpaths(ops):
        pixels = [
            (int(x * scale + offset_x), int(offset_y - y * scale))
            for (x, y) in sub
        ]
        if len(pixels) >= 3:
...
def score_cell(family: Family, glyph: str, wght: int) -> CellScores | None:
    ...
    donor_name = resolve_glyph_name(glyph, circular_otf(family, weight_name))
    glide_name = resolve_glyph_name(glyph, glide_vf(family))
