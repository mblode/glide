...
4. All worst scores < 0.3 → manual_review
   Nothing automated will fix this.
...
def recommend_for_glyph(
    ...
    if agg is None or agg["worstComposite"] is None:
        return Suggestion(
            ...
            confidence=0.1,
            reason="No scores available — glyph missing from donor or glide at all weights.",
        )
    ...
    if not cells:
        return Suggestion(
            strategy="manual_review",
            confidence=0.1,
            reason="No cell scores found for this glyph.",
        )
    ...
    if worst_comp < 0.3:
        return Suggestion(
            ...
            reason=(
                ...
                f"(composite {worst_comp:.2f}). No automated strategy is likely to fix this; "
                f"needs human design review."
            ),
        )
