...
def _instanced_glide(family: Family, wght: int) -> TTFont:
    base = load_font(glide_vf(family))
    return instancer.instantiateVariableFont(base, {"wght": wght})


@lru_cache(maxsize=64)
def _donor_font(family: Family, wght: int) -> TTFont:
    weight_name = next(w.name for w in CIRCULAR_WEIGHTS if w.wght == wght)
    return load_font(circular_otf(family, weight_name))
...
def _font_path(family: Family, source: CellSource, wght: int) -> Path:
    if source == "glide":
        return glide_vf(family)
    weight_name = next(w.name for w in CIRCULAR_WEIGHTS if w.wght == wght)
    return circular_otf(family, weight_name)


def main() -> int:
    parser = argparse.ArgumentParser(description="Render one glyph to SVG.")
    parser.add_argument("--family", choices=["roman", "italic"], required=True)
    parser.add_argument(
        "--glyph",
        required=True,
    ...
    parser.add_argument(
        ...
        type=int,
        required=True,
        choices=[w.wght for w in CIRCULAR_WEIGHTS],
    )
