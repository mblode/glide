...
- Only overwrites the `strategy` field
...
def apply_changes(triage: dict, changes: list[Change]) -> tuple[int, int]:
    ...
    for c in changes:
        ...
        if existing is None:
            fam["glyphs"][c.glyph] = {"strategy": c.after}
            if c.notes:
...
def format_change(c: Change) -> str:
    if c.before is None:
        return f"  + {c.key:40s}  (new)                → {c.after}  [{c.source}]"
    if c.before == c.after:
        return f"  = {c.key:40s}  {c.before}  (no change)  [{c.source}]"
    return f"  ~ {c.key:40s}  {c.before}  →  {c.after}  [{c.source}]"
...
def main() -> int:
    ...
    no_change = [c for c in changes if c.before == c.after]
    real_changes = [c for c in changes if c.before != c.after]
